...
import { sql, Import } from '@/lib/db';
import { read, utils } from 'xlsx';

export const dynamic = 'force-dynamic';

export async function GET() {
  const rows = await sql`SELECT * FROM imports ORDER BY id DESC LIMIT 100` as Import[];
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await sql`INSERT INTO imports (file_name, data_json) VALUES (${file.name}, ${JSON.stringify(allData)}) RETURNING id` as { id: number }[];

    return NextResponse.json({ id: result[0].id, file_name: file.name, sheets: Object.keys(allData).length, rows: Object.values(allData).reduce((s, r) => s + r.length, 0) }, { status: 201 });
  } catch (e) {
